...
def run_and_profile(approach_name: str, decks: np.ndarray, pattern_len: int, output_dir: str) -> dict:
    ...
    if approach_name == 'Approach 1': process_R(decks, agg_df, pattern_len)
    elif approach_name == 'Approach 2': process_H(decks, db_conn, pattern_len); db_conn.commit()
    cpu_time_end = process.cpu_times()
    end_time = time.perf_counter()
    _, peak_mem = tracemalloc.get_traced_memory()
    tracemalloc.stop()
    print("Processing complete.")
    if approach_name == 'Approach 1': agg_df.to_csv(output_csv, index=False)
    elif approach_name == 'Approach 2': export_db_to_csv(db_conn, output_csv); db_conn.close(); os.remove(temp_db_path)
